...
def strategic_value(prop, for_player, extra_props_receiving, engine):
    """Estimate the strategic value of a property to for_player if they receive it
    together with extra_props_receiving in the same trade."""
    base = prop.price
    if prop.type == "property":
        group = COLOR_GROUPS.get(prop.color, [])
        will_own = {pos for pos in group
                    if engine.board_properties[pos].owner == for_player}
        # Add positions coming in from this trade
        receiving_positions = {p.pos for p in extra_props_receiving
                               if p.type == "property"}
        will_own_after = will_own | receiving_positions
        frac = len(will_own_after) / len(group)
        if frac >= 1.0:
            return base * 2.5   # completes monopoly
        elif frac >= 0.67:
            return base * 1.4
        elif frac >= 0.34:
            return base * 1.15
    elif prop.type == "railroad":
        owned = sum(1 for p in for_player.properties if p.type == "railroad")
        ...
        owned = sum(1 for p in for_player.properties if p.type == "utility")
